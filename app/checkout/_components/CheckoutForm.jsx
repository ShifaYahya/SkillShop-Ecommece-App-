...
const CheckoutForm = ({ amount }) => {
	...
	const handleSubmit = async (event) => {
		...
		// Create New Order
		createOrder();
        sendEmail();          //react emai for emails templates
		// Send an Email
		...
		const { client_secret } = await res.json(); // Extract client_secret from the response

const result = await stripe.confirmPayment({
    // `Elements` instance that was used to create the Payment Element
    clientSecret: client_secret, // Use the extracted client_secret
    elements,
    confirmParams: {
